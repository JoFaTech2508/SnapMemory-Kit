#!/usr/bin/env python3
"""
SnapMemory-Kit - legacy export
Downloads memories from the html/memories_history.html + signed-link export type
"""
...
class SnapchatMemoryDownloader:
    ...
    def parse_html(self):
        ...
        for row in rows[1:]:  # Skip header row
            ...
            # Unique id for this memory, used so two memories in the same second don't collide
            sid_match = re.search(r"[?&]sid=([^&]+)", url)
            sid = sid_match.group(1) if sid_match else url

            try:
                # strptime here just consumes the literal "UTC" text, it doesn't
                # attach a real timezone, so that's done explicitly on the next line.
        ...
        # Same-second captures share a filename, so suffix duplicates with _2, _3, ...
        # to stop them overwriting each other.
        seen_stems = {}
        for memory in self.memories:
            base_stem = memory['date_local'].strftime("%Y-%m-%d_%H%M%S")
            count = seen_stems.get(base_stem, 0)
            seen_stems[base_stem] = count + 1
            memory['unique_stem'] = base_stem if count == 0 else f"{base_stem}_{count + 1}"
    ...
    def create_filename(self, memory):
        """Create a filename based on date and time (unique per memory)"""
        ext = self.get_file_extension(memory['url'], memory['media_type'])
        return f"{memory['unique_stem']}.{ext}"
    ...
    def set_file_dates(self, filepath, local_date):
        ...
        # macOS also tracks a separate creation date from the modification date above
